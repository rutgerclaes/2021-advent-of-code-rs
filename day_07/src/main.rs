...
fn main() {
    ...
    let positions: Vector<u32> = contents
        .split(",")
        .map(|p| p.parse::<u32>())
        .try_collect()
        .unwrap();
...
fn part_one(positions: &Vector<u32>) -> Result<u64> {
    let (min, max) = positions
        .iter()
        .minmax()
        .into_option()
        .ok_or(AOCError::new_from_ref("Empty list of positions"))?;
...
fn fuel_for(positions: &Vector<u32>, position: u32) -> u64 {
...
fn incr_fuel_for(positions: &Vector<u32>, position: u32, current_min: u64) -> Option<u64> {
    let result: itertools::FoldWhile<u64> = positions.iter().fold_while(0 as u64, |sum, &pos| {
        let new_sum =
...
fn part_two(positions: &Vector<u32>) -> Result<u64> {
    let sorted: Vector<&u32> = positions.into_iter().sorted().collect();
